...
export class LoginPage extends AbstractPage {
    ...
    /** Methods **/
    async setAuthCookiesAndOpenAssetsPage(customer: object, url: string = '/trade/assets/all', form?: any) {
        ...
        await this.page.context().addCookies([
            {
                name: 'sid',
                value: form === 'registration' ? cookies.sid : cookie.parse(cookies[0]).sid,
                domain: '.stage-capitalix.com',
                path: '/'
            },
            {
                name: 'auth_id',
                value: form === 'registration' ? cookies.auth_id : cookie.parse(cookies[2]).auth_id,
                domain: '.stage-capitalix.com',
                path: '/'
            },
        ]);
